...
async def next_page(bot, query):
    ...
    if int(req) not in [query.from_user.id, 0]:
        return await query.answer("⚠️ ᴛʜɪs ᴘᴀɢᴇ ɪs ɴᴏᴛ ғᴏʀ ʏᴏᴜ\nᴘʟᴇᴀsᴇ ʀᴇǫᴜᴇsᴛ ʏᴏᴜʀ ᴏᴡɴ ❗", show_alert=True)
    try:
        offset = int(offset)
    except:
        offset = 0
    ...
    if n_offset == 0:
        btn.append(
            [InlineKeyboardButton("⌫ 𝘽𝘼𝘾𝙆", callback_data=f"next_{req}_{key}_{off_set}"), InlineKeyboardButton(f"ᴘᴀɢᴇs {round(int(offset)/10)+1} / {round(total/10)}", callback_data="pages")]
        )
    elif off_set is None:
        btn.append([InlineKeyboardButton(f"ᴘᴀɢᴇs {round(int(offset)/10)+1} / {round(total/10)}", callback_data="pages"), InlineKeyboardButton("𝙉𝙀𝙓𝙏 ➪", callback_data=f"next_{req}_{key}_{n_offset}")])
    else:
        btn.append(
            [
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        buttons = [[
            ...
            InlineKeyboardButton('📢 𝖴𝗉𝖽𝖺𝗍𝖾𝗌', url='https://t.me/mf_linkz')
            ],[
...
async def auto_filter(client, msg, spoll=False):
    ...
    if offset != "":
        ...
        btn.append(
            [InlineKeyboardButton(text=f"ᴘᴀɢᴇs 1/{round(int(total_results)/10)}",callback_data="pages"), InlineKeyboardButton(text="𝙉𝙀𝙓𝙏 ➪",callback_data=f"next_{req}_{key}_{offset}")]
        )
    else:
        btn.append(
...
async def advantage_spell_chok(msg):
    query = re.sub(r"\b(pl(i|e)*?(s|z+|ease|se|ese|(e+)s(e)?)|((send|snd|giv(e)?|gib)(\sme)?)|movie(s)?|new|latest|br((o|u)h?)*|^h(e)?(l)*(o)*|mal(ayalam)?|tamil|kurup|kurupp|kurrup|file|that|find|und(o)*|kit(t(i|y)?)?o(w)?|thar(o)*w?|kittum(o)*|aya(k)*(um(o)*)?|full\smovie|any(one)|with\ssubtitle)", "", msg.text, flags=re.IGNORECASE) # plis contribute some common words 
    ...
    await a.delete()
    r = await message.reply('No results')
